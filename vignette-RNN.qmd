...
ts_data <- ts(df$Adj.Close, frequency = 1, start = c(year(df$Date[1]), month(df$Date[1])))
...
```
The above RMSE value must be taken with some caution as this approach is convenient for time series forecasting, keep in mind that traditional model evaluation metrics (such as Mean Squared Error) might not be directly applicable in this context, as you are continually updating the model with new information. So we can create a visualization below to show how the RMSE changes over time to give us a better understanding.
